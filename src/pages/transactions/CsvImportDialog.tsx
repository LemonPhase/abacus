          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Select value={mapping.date} onValueChange={(v: string | null) => onMappingChange({ ...mapping, date: v ?? "" })}>
              ...
              <div className="grid gap-2">
                ...
                <Select value={mapping.amount} onValueChange={(v: string | null) => onMappingChange({ ...mapping, amount: v ?? "" })}>
                  <SelectTrigger><SelectValue placeholder="Select column" /></SelectTrigger>
                  <SelectContent>
                    {headers.map((h) => (
                      <SelectItem key={h} value={h}>{h}</SelectItem>
              ...
              <div className="grid gap-2">
                ...
                <Select value={mapping.description} onValueChange={(v: string | null) => onMappingChange({ ...mapping, description: v ?? "" })}>
                  <SelectTrigger><SelectValue placeholder="Select column" /></SelectTrigger>
                  <SelectContent>
                    {headers.map((h) => (
                      <SelectItem key={h} value={h}>{h}</SelectItem>
              ...
              <div className="grid gap-2">
                ...
                <Select value={mapping.type} onValueChange={(v: string | null) => onMappingChange({ ...mapping, type: v ?? "" })}>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => onStepChange("upload")}>Back</Button>
              <Button onClick={handleRemap} disabled={!mapping.date || !mapping.amount}>Preview</Button>
            </DialogFooter>
          </div>
        )}
        ...
        {step === "preview" && (
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label>Account</Label>
                <Select value={accountId} onValueChange={(v: string | null) => onAccountChange(v ?? "")} items={accounts.map((a) => ({ value: a.id, label: a.name }))}>
              ...
              <div className="grid gap-2">
                ...
                <Select value={categoryId} onValueChange={(v: string | null) => onCategoryChange(v ?? "")} items={categories.map((c) => ({ value: c.id, label: c.name }))}>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => onStepChange("map")}>Back</Button>
              <Button onClick={onImport} disabled={!accountId || !categoryId || mappedRows.length === 0}>

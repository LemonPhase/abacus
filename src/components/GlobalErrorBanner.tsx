...
export default function GlobalErrorBanner() {
  ...
  return (
    <div className="fixed top-4 right-4 z-50 flex flex-col gap-2 max-w-md w-full">
      {errors.map((err, i) => (
        <div key={i} className="bg-destructive text-destructive-foreground p-4 rounded-lg shadow-lg flex items-start gap-3 animate-in slide-in-from-top-2">
          <AlertCircle className="size-5 shrink-0 mt-0.5" />
          <div className="flex-1 text-sm font-medium">{err.msg}</div>
          <button onClick={err.clear} className="shrink-0 opacity-80 hover:opacity-100 transition-opacity">
            <X className="size-5" />
          </button>
        </div>
      ))}

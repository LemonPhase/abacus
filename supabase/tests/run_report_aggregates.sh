#!/usr/bin/env bash
# Report aggregate tests (issue #11) against real PostgreSQL.
#
# Modeled on run_ownership_tests.sh: bootstraps a private throwaway cluster
# when no server is reachable, applies ALL migrations in order (the report
# RPCs depend on the currency provenance columns and the composite-FK schema),
# installs Supabase-equivalent grants, then runs report_aggregates_tests.sql
# as the non-superuser `authenticated` role via SET ROLE + the
# app.test_user_id GUC. Finally re-applies the migration to prove idempotency.
#
# Usage: supabase/tests/run_report_aggregates.sh
...
if ! $PSQL "${PSQL_ARGS[@]}" -d "$ADMIN_DB" -tAc 'select 1' >/dev/null 2>&1; then
  ...
  if ! command -v initdb >/dev/null; then
    echo "SKIP: no reachable PostgreSQL and initdb not found; report aggregate DB tests not run" >&2
    exit 0
  fi
...
AGG_MIGRATION=20260918000002_report_aggregates.sql
...
install_grants() { # $1=db — Supabase projects configure equivalent default
  ...
  # Function execute is restated because the aggregate migration revokes PUBLIC
  # (and 20260917000005 revokes PUBLIC for its RPC).
...
echo "== 1. aggregate semantics ($T1) =="
new_db "$T1"
for m in "${MIGRATIONS[@]}"; do
  run_sql "$T1" -f "supabase/migrations/$m" >/dev/null
done
install_grants "$T1"
run_sql "$T1" -f supabase/tests/report_aggregates_tests.sql >/dev/null
echo "   semantics: OK (summary/monthly/category sums, provenance branches, currency param, budgets, RLS, anon rejection, tied-date pagination determinism)"

echo "== 2. idempotency ($T1) =="
run_sql "$T1" -f "supabase/migrations/$AGG_MIGRATION" >/dev/null
run_sql "$T1" -f supabase/tests/report_aggregates_tests.sql >/dev/null
echo "   idempotency: OK (migration re-applied cleanly, aggregates unchanged)"

echo "All report aggregate DB tests passed."
